...
public class FeatureDAO implements BaseDAO<FeatureModel> {

    @Override
    public FeatureModel getById(Long id) { return null; } // PK String, tidak dipakai
    ...
    @Override
    public boolean save(FeatureModel f) {
    ...
    @Override
    public boolean update(FeatureModel f) {
    ...
    public FeatureModel getByCode(String code) {
        String sql = "SELECT feature_code, feature_name, fee FROM m_feature WHERE feature_code=?";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, code);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return new FeatureModel(rs.getString("feature_code"), rs.getString("feature_name"), rs.getDouble("fee"));
            }
        } catch (SQLException e) { e.printStackTrace(); }
        return null;
    }
